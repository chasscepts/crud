  ...
  return (
    <div style={styles.screen}>
      ...
      <div style={styles.screenBody}>
        <iframe style={styles.iframe} srcDoc={html} title={id} />
      </div>
    </div>
  );
};
...
const Notice = ({ notification }) => {
  ...
  useEffect(() => {
    ...
    if (state.closed) {
      setTimeout(() => manager.remove(id), 400);
    }
  }, [state]);

  const close = () => setState({ loaded: true, closed: true });
  ...
  return (
    ...
    >
      <div style={{ ...styles.feedback, ...style }}>{text}</div>
